...
import pickle
import matplotlib
# Set a non-interactive backend to prevent GUI-related attribute errors in some environments
matplotlib.use('Agg')
...
# Scaler is not required as the model was trained on raw data
scaler = None
...
if page == "Prediction":
    ...
    with col2:
        ...
        # Formula: (Likes + Comments) / (Views + 1) to match training logic
        ...
        if st.button("Calculate Revenue"):
            if model is not None:
                if views == 0:
                    ...
                    # Prepare input data dictionary with all necessary columns
                    input_data = {
                        'views': views,
                        'likes': likes,
                        'comments': comments,
                    ...
                    # Make Prediction directly on raw features
